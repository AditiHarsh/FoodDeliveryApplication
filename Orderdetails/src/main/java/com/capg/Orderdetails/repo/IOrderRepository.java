package com.capg.Orderdetails.repo;
...
//			+ " (select item_id from restaurant_item_table where restaurant_id in "
//			+ "(select restaurant_id from restaurant where restaurant_id=?1)))", nativeQuery = true)
//	public List<OrderDetails> viewAllOrdersRes(int i);

	@Query(value = "select * from order_details where cart_id in " + 
			"(select cart_id from food_cart where customer_customer_id  in" + 
			" (select customer_id from customer where customer_id=?1))"
			, nativeQuery = true)
	public List<OrderDetails> viewAllOrders(int i);
